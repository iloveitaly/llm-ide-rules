"""Tests for Grok CLI agent integration."""
...
def test_grok_write_rule():
    """Test that GrokAgent writes rules with correct YAML frontmatter."""
    agent = GrokAgent()
...
def test_grok_write_command():
    """Test that GrokAgent writes commands/skills with correct format."""
    agent = GrokAgent()
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
...
"""
            ...
            explode_result = runner.invoke(
                app, ["explode", "instructions.md", "--agent", "grok"]
            )
            ...
            implode_result = runner.invoke(app, ["implode", "grok", "roundtrip.md"])
...
def test_roundtrip_grok_commands():
    """Test explode -> implode grok produces equivalent commands.md."""
    runner = CliRunner()
...
"""
            ...
            implode_result = runner.invoke(app, ["implode", "grok"])
            assert implode_result.exit_code == 0
