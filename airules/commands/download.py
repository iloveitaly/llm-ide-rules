...
DEFAULT_REPO = "iloveitaly/llm-ide-prompts"
...
def copy_instruction_files(repo_dir: Path, instruction_types: List[str], target_dir: Path):
    ...
    for inst_type in instruction_types:
        ...
        for dir_name in config["directories"]:
            ...
            if source_dir.exists():
                logger.info("Copying directory", source=str(source_dir), target=str(target_subdir))
                
                # Create target directory
                target_subdir.mkdir(parents=True, exist_ok=True)
                
                # Copy all files from source to target
                copy_directory_contents(source_dir, target_subdir, config.get("exclude_patterns", []))
        ...
        for file_name in config["files"]:
            ...
            if source_file.exists():
                logger.info("Copying file", source=str(source_file), target=str(target_file))
...
def copy_directory_contents(source_dir: Path, target_dir: Path, exclude_patterns: List[str]):
...
def download_main(
    instruction_types: Annotated[List[str], typer.Argument(help="Types of instructions to download (cursor, github, gemini, claude, agent). Downloads everything by default.")] = None,
    repo: Annotated[str, typer.Option("--repo", "-r", help="GitHub repository to download from")] = DEFAULT_REPO,
    branch: Annotated[str, typer.Option("--branch", "-b", help="Branch to download from")] = DEFAULT_BRANCH,
    target_dir: Annotated[str, typer.Option("--target", "-t", help="Target directory to download to")] = ".",
    ...
    if invalid_types:
        logger.error("Invalid instruction types", invalid_types=invalid_types, valid_types=list(INSTRUCTION_TYPES.keys()))
        raise typer.Exit(1)
