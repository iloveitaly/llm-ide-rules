...
def bundle_cursor_rules(rules_dir, output_file):
    rule_files = sorted(Path(rules_dir).glob("*.mdc"))
...
def bundle_github_instructions(instructions_dir, output_file):
    instr_files = sorted(Path(instructions_dir).glob("*.instructions.md"))
    with open(output_file, "w") as out:
...
def main():
    ...
    parser.add_argument("output", nargs="?", default="instructions.bundled.md", help="Output file")
